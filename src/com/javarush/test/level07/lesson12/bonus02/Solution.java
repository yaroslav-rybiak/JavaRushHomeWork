...
public class Solution
{
    public static void main(String[] args) throws IOException
    {
        ...
        ArrayList<String> newShit = new ArrayList<String>();
        for (int i = 0; i < list.size(); i++)
        {
            if (list.get(i).length() % 2 == 0) {
                newShit.add(list.get(i) + " " + list.get(i));
            }
            else newShit.add(list.get(i) + " " + list.get(i) + " " + list.get(i));
        }

        for (int i = 0; i < newShit.size(); i++)
        {
            System.out.println(newShit.get(i));
        }
    }
}
